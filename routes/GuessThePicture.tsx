...
export const GuessThePicture = ({ route, navigation }) => {
  ...
  return (
    <View style={styles.container}>
      <Polaroid imageSource={{ uri: picture.img }} text={chosenUser} />
      <Card style={styles.usercard}>
        {userData.room.users.map((mappedUser) => {
          if (mappedUser.userID !== userData.user.userID) {
            return (
              <Chip
                key={mappedUser.userID}
                style={styles.chip}
                onPress={() => {
                  setChosenUserID(mappedUser.userID);
                  userData.room.users.find((user) => {
                    if (user.userID === chosenUserID) {
                      setChosenUser(user.username);
                    }
                  });
                }}
              >
      ...
      <Button disabled={hasSubmitted} onPress={submitGuess}>
        Submit
      </Button>
    </View>
  );
