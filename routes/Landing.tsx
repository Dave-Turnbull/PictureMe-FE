...
import stripedpaper from "../assets/striped-paper-texture.jpg";
...
const Landing = ({ navigation }) => {
  ...
  const createGame = async () => {
    ...
    const usersInRoom = [{ username: username, id: "1" }];
    const roomObject = await new Promise((resolve) => {
      socket.emit(
        "createRoom",
        { username, userID: userData.user.id },
        (message, roomObj) => {
          resolve(roomObj);
        }
      );
      ...
      >
        ...
        <View style={styles.userInput}>
          <StyledTextInput
            ...
            label="username..."
            value={username}
            onChangeText={(username: SetStateAction<string>) => {
          ...
          >
            ...
            <Text>
              {" "}
              - Each player will take a picture when given a prompt at the
              beginning of each round
            </Text>
            <Text>
              {" "}
              - Each picture will be displayed and each player will need to
              guess who's pic is whos, Best guesses only!
            </Text>
          </Modal>
